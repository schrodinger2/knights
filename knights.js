...
function movingKnights() {
  ...
  for (var i = 0; i < knights.length; i++) {
    const squareID = knights[i].parentNode.classList[2];
    //the idintifier class has the position of the square (y and i)

    const jth = squareID[1]; //j of the current parentNode
    const ith = squareID[3]; //i of the current parentNode

    const newJAndI = randomMoves(jth, ith);
    const newParentNode = document.body.querySelector(`.j${newJAndI[0]}i${newJAndI[1]}`);
